...
const verifyEmail = async (req, res) => {
  ...
  try {
    const userRecords = await UserOTP.find({ userId });

    if (userRecords.length <= 0) {
      return res.status(httpStatus.BAD_REQUEST).json({ msg: "Account record doesn't exist or has already been verified" });
    }
    ...
    sendEmail(req.user.email, 'Signup Success', emailTemplate.signupSuccess()).catch((error) => {
